...
public class PokemonPlante extends Pokemon {
	...
	public void attaquer(PokemonPlante p) {
        ...
        public String toString(){
            return this.nom.toUpperCase() + " ( HP = " + this.hp + ", ATTAQUE = " + this.atk + " TYPE = PLANTE )" ;
        }
        @Override
        public void afficher() {
		System.out.println("\t\t\t                    |\t\t        |\t\t        ");
                if(!isDead()){
                    System.out.println("\t\t\t          " + String.format("%-10s", this.nom) + "|\t\t" + String.format("%4.2f", this.hp) + "  |\t\t" +this.atk + "\t PLANTE");
                } else {
                    System.out.println("\t\t\t          " + String.format("%-10s", this.nom) + "|\t\t" + String.format("%3.2f", this.hp) + "  |\t\t" +this.atk + "\t PLANTE" + "\t\t\tMORT");
                }
		System.out.println("\t\t\t                    |\t\t        |\t\t        ");
	}
